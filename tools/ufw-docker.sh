...
if grep -q "^\[Peer\]" ~/dwg-dark/wireguard/wg0.conf; then
  printf "${GREEN}В файле ~/dwg-dark/wireguard/wg0.conf есть клиенты. Можно продолжать работу.${NC}\n"
else
  printf "${BG_RED}В файле ~/dwg-dark/wireguard/wg0.conf нет клиентов. Нельзя продолжать работу.${NC}\n"
